...
Participant-facing copy of one frozen report. This copy intentionally uses only the neutral A label.

## Scope and evidence

This report summarizes the same 30-game frozen set used by the other report. It uses the mistake, blunder, inaccuracy, evaluation, and best-move annotations embedded in the supplied PGN. The annotation source identifies itself as Lichess, but engine identity, version, settings, and limits are unknown.
...
This recommendation is a conventional summary. It does not claim that all marked errors share one player-level cause.
