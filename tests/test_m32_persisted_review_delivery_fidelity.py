...
from chess_mentor_engine.participant_review_package import build_participant_review_package
...
_GOLDEN_PATH = Path(__file__).parent / "fixtures" / "m32_delivery_semantic_golden.json"
...
def _m22_read_model(scenario: str) -> tuple[dict, dict]:
    *_, presentation = m22._scenario(scenario)
    read_model = build_coach_review_read_model(evaluation_presentation=presentation)
...
def test_m22_matrix_survives_delivery_without_semantic_reinterpretation(case) -> None:
    ...
    assert m22._presentation_signature(delivered) == m22._presentation_signature(presentation)
...
def test_persisted_package_build_is_deterministic_and_exactly_source_bound(tmp_path) -> None:
    ...
    assert first["source_fingerprints"]["m25_read_model_fingerprint"] == run.read_model["fingerprint"]
    assert first["source_fingerprints"]["m25_source_fingerprints"] == run.read_model["source_fingerprints"]
    assert first["delivery"]["sections"]["objective_evidence"]["content"] == run.read_model["objective_evidence"]
    assert first["delivery"]["sections"]["model_coaching"]["present"] is False
    assert validate_review_delivery_bundle(first) == first
    assert validate_persisted_review_delivery_bundle(
        store=store,
        participant_id="P01",
        bundle=first,
    ) == first
...
def test_model_content_remains_separate_from_objective_and_evaluator_truth(tmp_path) -> None:
    ...
    assert sections["objective_evidence"]["authority"] == "objective_chess_evidence"
    assert sections["deterministic_grounding"]["authority"] == "deterministic_grounded_feedback"
    assert sections["model_coaching"]["authority"] == "model_language_request_bound_not_semantically_verified"
    assert sections["model_evaluation"]["authority"] == "bounded_model_output_quality_assessment_not_objective_truth"
    assert sections["model_coaching"]["content"] == run.read_model["model_coaching"]
    assert sections["model_evaluation"]["content"] == run.read_model["model_evaluation"]
    assert sections["model_evaluation"]["content"]["truth_status"] == "not_established_by_m20_evaluation"
    assert bundle["authority_boundary"]["promotes_model_language_to_fact"] is False
    assert bundle["authority_boundary"]["promotes_evaluator_judgment_to_objective_truth"] is False


def test_rehashed_decision_perspective_inversion_is_rejected() -> None:
    _, read_model = _m22_read_model("bounded_black_perspective")
    tampered = copy.deepcopy(read_model)
    tampered["objective_evidence"]["score_semantics"]["display_perspective"] = "white"
    tampered["source_fingerprints"]["objective_evidence"] = _fingerprint(tampered["objective_evidence"])
...
def test_rehashed_bound_loss_and_numericized_mate_are_rejected() -> None:
    ...
    lost_bound["source_fingerprints"]["objective_evidence"] = _fingerprint(lost_bound["objective_evidence"])
    ...
    numericized["objective_evidence"]["comparison"]["best_evaluation"]["centipawns"] = 100000
    numericized["source_fingerprints"]["objective_evidence"] = _fingerprint(numericized["objective_evidence"])
...
def test_rehashed_candidate_reordering_and_source_fingerprint_drift_are_rejected() -> None:
    ...
    reordered["source_fingerprints"]["objective_evidence"] = _fingerprint(reordered["objective_evidence"])
    ...
    with pytest.raises(ReviewDeliveryFidelityError, match="source fingerprint|fingerprint"):
...
def test_rehashed_authority_promotion_is_rejected(tmp_path) -> None:
    ...
    tampered["delivery"]["sections"]["model_coaching"]["authority"] = "objective_chess_evidence"
    tampered = _rehash_bundle(tampered)

    with pytest.raises(ReviewDeliveryFidelityError, match="model_coaching authority"):
        validate_review_delivery_bundle(tampered)


def test_persisted_validation_rejects_rehashed_package_source_swap_and_wrong_participant(tmp_path) -> None:
    store, _, run = _deterministic_run(tmp_path)
    package = build_participant_review_package(
        store=store,
        participant_id="P01",
    ...
    tampered = copy.deepcopy(bundle)
    tampered["source_refs"]["m30_package"]["artifact_id"] = "participant_review_package_forged"
    tampered = _rehash_bundle(tampered)
    with pytest.raises(ReviewDeliveryFidelityError, match="M30 package validation failed"):
        validate_persisted_review_delivery_bundle(
            store=store,
            participant_id="P01",
            bundle=tampered,
    ...
    with pytest.raises(ReviewDeliveryFidelityError, match="requested participant"):
        validate_persisted_review_delivery_bundle(
            store=store,
