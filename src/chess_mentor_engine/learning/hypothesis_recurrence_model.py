...
ReviewState: TypeAlias = Literal["not_required", "completed"]
CompetingExplanationReviewState = ReviewState
ChallengeReviewKind: TypeAlias = Literal[
    "contradiction",
    "successful_counterexample",
]
...
_CHALLENGE_KINDS = frozenset(
    {"contradiction", "successful_counterexample"}
)
...
class HypothesisAssessmentPolicyRef:
    """Exact material identity for one M7C recurrence policy."""
...
class HypothesisAssessmentPolicy:
    """Versioned material participant-specific recurrence policy."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.m6_policy_compatibility_rule == "same_policy_fingerprint":
            if self.compatible_m6_policy_fingerprints:
                raise ValueError(
                    "same_policy_fingerprint cannot declare compatible policies"
                )
        elif not self.compatible_m6_policy_fingerprints:
            raise ValueError(
                "declared_policy_fingerprints requires compatible policies"
            )
        if self.stage_compatibility_rule == "same_assessed_stage_ids":
            if self.allowed_stage_ids:
                raise ValueError(
                    "same_assessed_stage_ids cannot declare allowed stages"
                )
...
class CompetingExplanationReview:
    """Provenance that alternatives were considered, not refuted."""
    ...
    def __post_init__(self) -> None:
        ...
        _require_unique_strings(
            ...
            self.reviewed_alternative_notes,
        )
        ...
        if self.state == "not_required" and self.reviewer_provenance is not None:
            raise ValueError("not_required review cannot carry reviewer provenance")
...
@dataclass(frozen=True, slots=True)
class HypothesisChallengeReview:
    """Provenance that contradiction/counterexample evidence was reviewed."""

    review_id: str
    fingerprint: str
    kind: ChallengeReviewKind
    state: ReviewState
    reviewed_link_refs: tuple[HypothesisEvidenceLinkRef, ...]
    review_note: str
    reviewer_provenance: HypothesisActorProvenance | None
    created_at: str

    def __post_init__(self) -> None:
        if self.kind not in _CHALLENGE_KINDS:
            raise ValueError("unknown hypothesis challenge review kind")
        if self.state not in _REVIEW_STATES:
            raise ValueError("unknown hypothesis challenge review state")
        for name, value in (
            ("review_id", self.review_id),
            ("fingerprint", self.fingerprint),
            ("review_note", self.review_note),
            ...
            _require_nonempty(name, value)
        ids = tuple(item.link_id for item in self.reviewed_link_refs)
        if len(set(ids)) != len(ids):
            raise ValueError("reviewed_link_refs must be unique")
        if self.state == "completed" and self.reviewer_provenance is None:
            raise ValueError("completed review requires reviewer provenance")
        if self.state == "not_required" and self.reviewer_provenance is not None:
            raise ValueError("not_required review cannot carry reviewer provenance")
    ...
    def to_dict(self, *, include_identity: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            ...
            "state": self.state,
            "reviewed_link_refs": [
                item.to_dict() for item in self.reviewed_link_refs
            ],
            "review_note": self.review_note,
            "reviewer_provenance": (
                None
...
class HypothesisRecurrenceUnit:
    """One participant-position unit after deterministic link aggregation."""
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            item not in _MEASUREMENT_CONDITIONS
            for item in self.measurement_conditions
        ):
            raise ValueError("unknown recurrence unit measurement condition")
        if len(set(self.measurement_conditions)) != len(
...
class HypothesisEvidenceSummary:
    """Structured evidence counts and exclusions, never a weakness score."""
    ...
    def __post_init__(self) -> None:
        counts = (
            self.eligible_link_count,
            self.excluded_link_count,
            self.support_unit_count,
            self.independent_support_count,
            self.contradiction_unit_count,
            self.successful_counterexample_unit_count,
            self.context_exception_unit_count,
            self.unclear_unit_count,
            self.mixed_unit_count,
        )
        if any(value < 0 for value in counts):
            raise ValueError("evidence summary counts must not be negative")
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "excluded_link_refs": [
                item.to_dict() for item in self.excluded_link_refs
            ],
            "exclusion_reasons": [
                [link_id, list(reasons)]
                for link_id, reasons in self.exclusion_reasons
            ],
        }
...
class HypothesisAssessment:
    """Immutable assessment of one exact revision and frozen evidence set."""
    ...
    def __post_init__(self) -> None:
        ...
        actual_unit_ids = tuple(
            item.recurrence_unit_id for item in self.recurrence_units
        )
        if actual_unit_ids != self.recurrence_unit_ids:
            raise ValueError("recurrence units and IDs must align")
        if any(
            item not in _MEASUREMENT_CONDITIONS
            for item in self.measurement_conditions
        ):
    ...
    def to_dict(self, *, include_identity: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            ...
            "status": self.status,
            "support_link_refs": [
                item.to_dict() for item in self.support_link_refs
            ],
            ...
            "successful_counterexample_link_refs": [
                item.to_dict()
                for item in self.successful_counterexample_link_refs
            ],
            "context_exception_link_refs": [
                item.to_dict() for item in self.context_exception_link_refs
            ],
            "unclear_link_refs": [
                item.to_dict() for item in self.unclear_link_refs
            ],
            "recurrence_units": [
                item.to_dict() for item in self.recurrence_units
            ],
            ...
            "source_game_ids": list(self.source_game_ids),
            "measurement_conditions": list(self.measurement_conditions),
            "contradiction_review": self.contradiction_review.to_dict(),
            "counterexample_review": self.counterexample_review.to_dict(),
            "competing_explanation_review": (
                self.competing_explanation_review.to_dict()
            ),
...
class HypothesisAssessmentRef:
    """Exact reference to one immutable M7C assessment."""
...
class HypothesisLedgerEntry:
    """One derived current-state entry over append-only history."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.authority_lifecycle_state == "active":
            if self.latest_lifecycle_event_ref is not None:
                raise ValueError("active entry cannot cite a lifecycle event")
        else:
            if self.latest_lifecycle_event_ref is None:
                raise ValueError("terminal entry requires a lifecycle event")
            if (
                self.latest_lifecycle_event_ref.kind
                != self.authority_lifecycle_state
            ):
                raise ValueError("lifecycle state does not match event kind")
...
class HypothesisLedgerSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            ...
            raise ValueError(
                "ledger entries must belong to the snapshot participant"
            )
