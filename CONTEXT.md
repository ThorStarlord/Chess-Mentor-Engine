...
M6 — Reasoning Discrepancy — is in progress with **M6A contract frozen and M6B deterministic comparison facts qualified**. The frozen production boundary in `docs/architecture/reasoning-discrepancy.md` and ADR 0005 defines Reasoning Discrepancy as a position-local, evidence-bound comparison between qualified objective evidence and frozen Player Decision Evidence without promoting unreported ideas into hidden cognition. M6A separates deterministic `DiscrepancyFact` relations from append-only human/model `ReasoningCoding`, preserves `match / conflict / not_explicitly_reported / ambiguous / not_observed / not_comparable`, keeps engine judgment distinct from deterministic chess truth, preserves A1/A2 stage and M5 measurement-condition provenance, and freezes the rule `not explicitly reported != not considered != not recognized != not generated`. M6B now implements the deterministic layer in `src/chess_mentor_engine/learning/`: strict `ReasoningDiscrepancyContext` binding to exact M4 candidate/batch/comparison/signal evidence plus exact frozen M5 stages, and stage-specific `DiscrepancyFact` derivation for explicitly structured selected move, candidate membership, expected reply, and expected continuation. M6B preserves `clean / instrument_aware_clean / deviating / contaminated` measurement conditions, never parses raw prose, treats missing structured dimensions as `not_observed`, preserves ambiguous reports as `ambiguous`, uses deterministic chess legality for genuine illegal-move conflicts, and treats legal replies/continuations that merely diverge from one non-forced engine PV as `not_comparable` rather than automatically wrong. Missing or mismatched comparable engine evidence remains `not_comparable` or is rejected. **M6C — coded local discrepancy assessment only — is authorized next; M6Q has not started and M7 remains unauthorized.** See [the M6A architecture contract](docs/architecture/reasoning-discrepancy.md), [the M6B qualification record](docs/architecture/reasoning-discrepancy-facts.md), [ADR 0005](docs/decisions/0005-reasoning-discrepancy-contract.md), and [the current build-status record](docs/product/repository-build-status.md).
