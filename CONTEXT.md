...
M7 — Learner Hypothesis Ledger — is **in progress with M7A contract frozen, M7B immutable hypothesis/evidence ledger qualified, and M7C recurrence assessment + derived ledger state qualified**. M7A is the first production boundary allowed to reason across multiple qualified M6 position-local evidence units and freezes participant-specific `descriptive_pattern` hypotheses, explicit evidence relations, versioned recurrence policy, recurrence assessment states, append-only lifecycle, and the one-participant/canonical-position-one-recurrence-unit rule. M7B implements stable hypothesis lineage identity, append-only revisions/lifecycle history, and provenance-bound evidence mappings. M7C now implements materially versioned `HypothesisAssessmentPolicy`, exact M6-policy/stage/measurement/context eligibility and compatibility gates, one participant-position recurrence units, explicit independence rules, provenance-bound contradiction/counterexample/competing-explanation review, deterministic `HypothesisAssessment`, structured exclusions/evidence summaries, and rebuildable `HypothesisLedgerSnapshot` state while preserving recurrence status separately from `active / retired / superseded` authority lifecycle. The exact M7C implementation head `e6f6debee79ea7780b5dbfc1732def802d9bd8b4` passed `294` tests with `8` intentional external-engine skips, all `38` focused M7C tests, Ruff, and Stockfish in Actions run `34311289575`; PR #33 merged it tree-identically as `df6bee774259ac98e8edbcdd2ab17306c9c09873`, and post-merge run `34311520576` passed both jobs. M7C can assign bounded participant-specific recurrence states under an exact frozen policy/evidence set, but `supported_recurrence` remains a descriptive policy result rather than a causal cognitive mechanism, permanent learner trait, universal weakness score, training-eligibility decision, or pedagogical prescription. **M7Q — full M7 qualification — is the only next authorized milestone slice. M8 remains unauthorized until M7Q passes and repository status is explicitly reconciled.** See [the M7A architecture contract](docs/architecture/learner-hypothesis-ledger.md), [the M7B implementation/qualification record](docs/architecture/learner-hypothesis-evidence-ledger.md), [the M7C implementation/qualification record](docs/architecture/learner-hypothesis-recurrence-assessment.md), [ADR 0006](docs/decisions/0006-learner-hypothesis-ledger-contract.md), and [the current build-status record](docs/product/repository-build-status.md).
