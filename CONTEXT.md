...
M5 — Player Decision Evidence — is now in progress at the contract boundary only. M5A is frozen in `docs/architecture/player-decision-evidence.md` and ADR 0004. The contract crosses from objective chess evidence into direct participant-reported evidence while preserving the separation `objective chess truth != participant self-report != analyst/model coding != learner diagnosis`. It requires verbatim raw-response preservation, versioned prompt/presentation provenance, separate minimal/probe/post-reveal/recollection stages, first-class immutable freeze events, explicit objective-evidence reveal timing, exposure and instrument-awareness provenance, append-only amendments, and preservation of ambiguous/illegal reported moves and contaminated/deviating evidence rather than silent correction or erasure. Pilot 003/004 instrument boundaries informed the contract but remain immutable research artifacts and are not treated as universal product prompt truth. M5 production entities are not implemented yet. M5B — the immutable evidence model only — is authorized next; M5C and M6 remain unauthorized. See [the M5 architecture contract](docs/architecture/player-decision-evidence.md), [ADR 0004](docs/decisions/0004-player-decision-evidence-contract.md), and [the current build-status record](docs/product/repository-build-status.md).
