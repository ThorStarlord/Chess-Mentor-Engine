...
M6 — Reasoning Discrepancy — is now in progress with **M6A contract frozen and implementation not started**. The frozen production boundary in `docs/architecture/reasoning-discrepancy.md` and ADR 0005 defines a Reasoning Discrepancy as a position-local, evidence-bound comparison between qualified objective evidence and frozen Player Decision Evidence without promoting unreported ideas into hidden cognition. M6A separates deterministic `DiscrepancyFact` relations from append-only human/model `ReasoningCoding`, preserves `match / conflict / not_explicitly_reported / ambiguous / not_observed / not_comparable`, keeps engine judgment distinct from deterministic chess truth, preserves A1/A2 stage and M5 measurement-condition provenance, and freezes the rule `not explicitly reported != not considered != not recognized != not generated`. It also defines immutable local assertion/assessment records and a versioned assessment policy, while explicitly deferring recurrence, stable learner weakness, causal cognitive traits, learner hypotheses, and pedagogy to later milestones. **M6B — deterministic reasoning-evidence comparisons only — is authorized next; M6C/M6Q have not started and M7 remains unauthorized.** See [the M6A architecture contract](docs/architecture/reasoning-discrepancy.md), [ADR 0005](docs/decisions/0005-reasoning-discrepancy-contract.md), and [the current build-status record](docs/product/repository-build-status.md).
