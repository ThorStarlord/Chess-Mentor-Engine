...
M3 — Engine Evidence — has a frozen normalized contract under ADR 0002. M3B is qualified and merged: the repository now provides provider-neutral engine-evidence records, request/result fingerprints, a `ChessAnalysisProvider` boundary, and a fixture-backed `PrecomputedAnalysisProvider` that validates candidate root moves and complete principal variations against the qualified chess-rules substrate. M3C external UCI/Stockfish integration has not started, so M3 as a whole is not yet qualified. No engine binary is bundled. See [the M3 architecture note](docs/architecture/engine-evidence.md) and [ADR 0002](docs/decisions/0002-engine-evidence-contract.md).
