...
M6 — Reasoning Discrepancy — is in progress with **M6A contract frozen, M6B deterministic comparison facts qualified, and M6C coded local discrepancy assessment qualified**. The frozen production boundary in `docs/architecture/reasoning-discrepancy.md` and ADR 0005 defines Reasoning Discrepancy as a position-local, evidence-bound comparison between qualified objective evidence and frozen Player Decision Evidence without promoting unreported ideas into hidden cognition. M6A separates deterministic `DiscrepancyFact` relations from append-only human/model `ReasoningCoding` and freezes `not explicitly reported != not considered != not recognized != not generated`. M6B implements strict `ReasoningDiscrepancyContext` binding and deterministic stage-specific facts for explicitly structured selected move, candidate membership, expected reply, and expected continuation while preserving `match / conflict / not_explicitly_reported / ambiguous / not_observed / not_comparable`, M5 measurement condition, and the distinction between deterministic chess legality and engine judgment. M6C now implements immutable `ReasoningCoding`, versioned `ReasoningAssessmentPolicy`, conservative `ReasoningDiscrepancyAssertion`, and `ReasoningDiscrepancyAssessment` records with explicit `discrepancy_supported / no_supported_discrepancy / unclear / unscorable` states. Human/model coding remains separate from M5 participant evidence and M6B facts; arbitrary coder prose is not promoted into assertion wording; A1/A2 remain distinct evidence stages; same-code/same-stage disagreement is retained as uncertainty rather than last-write-wins; deviating/contaminated evidence is assessable only when explicitly allowed by policy; and assessment-time provenance revalidation prevents reconstructed records from substituting unrelated player/objective/fact evidence. All M6C claims remain position-local: no recurrence, stable learner weakness, causal cognitive trait, learner hypothesis, or pedagogy is established. **M6Q — full Reasoning Discrepancy qualification only — is authorized next; it has not started, and M7 remains unauthorized.** See [the M6A architecture contract](docs/architecture/reasoning-discrepancy.md), [the M6B qualification record](docs/architecture/reasoning-discrepancy-facts.md), [the M6C qualification record](docs/architecture/reasoning-discrepancy-assessment.md), [ADR 0005](docs/decisions/0005-reasoning-discrepancy-contract.md), and [the current build-status record](docs/product/repository-build-status.md).
