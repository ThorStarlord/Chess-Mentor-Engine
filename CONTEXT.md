...
M7 — Learner Hypothesis Ledger — is **in progress with M7A contract frozen and no production M7 implementation yet**. The frozen M7A contract in `docs/architecture/learner-hypothesis-ledger.md` and ADR 0006 is the first production boundary allowed to reason across multiple qualified M6 position-local evidence units. It freezes participant-specific `descriptive_pattern` hypotheses with stable lineage identity, append-only revisions, explicit `HypothesisEvidenceLink` relations (`supports / contradicts / successful_counterexample / context_exception / unclear`), versioned cross-position assessment policy, recurrence assessment states (`insufficient / isolated / candidate_recurrence / supported_recurrence / contradicted / unclear`), separate `active / retired / superseded` authority lifecycle, and rebuildable ledger snapshots. One participant/canonical position is one recurrence unit regardless of multiple assertions, coders, stages, or repeated assessments. Recurrence requires more than one distinct qualifying position plus an explicit independence rule and traceable common structure. M4 operational controls and M6 `no_supported_discrepancy` are **not** automatically contradictions or successful counterexamples; they become counterevidence only when the exact hypothesis-relevant dimension was observed, assessable, comparable, context-matching, and explicitly mapped under a versioned M7 rule/coding. Supported recurrence remains a bounded descriptive inference, not a causal cognitive mechanism, permanent trait, training-eligibility judgment, intervention prescription, or learning/mastery claim. M7A freezes no universal weakness/confidence scalar, no embedding/LLM similarity authority, and no pedagogy. **M7B — immutable hypothesis/evidence ledger — is the only next authorized implementation slice; M7C recurrence assessment and M7Q remain later M7 gates, and M8 remains unauthorized.** See [the M7A architecture contract](docs/architecture/learner-hypothesis-ledger.md), [ADR 0006](docs/decisions/0006-learner-hypothesis-ledger-contract.md), and [the current build-status record](docs/product/repository-build-status.md).
