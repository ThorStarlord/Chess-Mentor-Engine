...
M4A — Diagnostic Position Selection contract — is frozen, but M4 implementation has not started. ADR 0003 separates objective `DecisionComparison`, transparent `SelectionSignal`, versioned `SelectionPolicy`, `DiagnosticCandidate`, and bounded `DiagnosticCandidateBatch` records. The frozen contract requires compatible complete exact engine evidence for initial quantitative comparisons, preserves mate as semantic winner-plus-plies evidence, treats partial/failure/bound evidence conservatively, makes successful/control decisions first-class, and prohibits learner-psychology or pedagogical claims. The next authorized task is M4B `DecisionComparison` only. See [the M4 architecture note](docs/architecture/diagnostic-position-selection.md), [ADR 0003](docs/decisions/0003-diagnostic-position-selection-contract.md), and [the current build-status record](docs/product/repository-build-status.md).
